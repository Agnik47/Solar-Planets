  ...
  //add a Throtal MouseMove event to the scene
  ...
  window.addEventListener("wheel", (event) => {
    ...
    if (currentTime - lastWheelEventTime >= throttleDelay) {
      ...
      // Your throttled wheel event logic here
      const direction = event.deltaY > 0 ? "down" : "up";
      const headings = document.querySelectorAll(".headings");

      scrollCounter = (scrollCounter + 1) % 4;
  ...
  function animate() {
    ...
    spheres.children.forEach((sphere) => {
      sphere.rotation.y += 0.0005; // Adjust rotation speed here
    });
